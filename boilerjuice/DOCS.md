...
- **Modern Web UI** — View your tank level with an animated gauge, stats cards, and responsive layout. Works on desktop and mobile.
...
- **Automatic Data Refresh** — Configurable polling interval (15 min to 24 hours) to keep your tank data up-to-date.
...
- **Configurable Tank Capacity** — Set your actual tank size in litres so remaining oil is calculated accurately from the percentage.
...
3. Enter your BoilerJuice email, password, Tank ID, and tank capacity (litres).
...
If you have an MQTT broker (like Mosquitto), enable MQTT in Settings. The add-on will automatically create these sensors:

| Sensor | Entity ID | Unit |
|--------|-----------|------|
| Oil Remaining | `sensor.boilerjuice_oil_remaining` | L |
| Oil Level | `sensor.boilerjuice_oil_percentage` | % |
| Tank Capacity | `sensor.boilerjuice_tank_capacity` | L |

All sensors appear under the **BoilerJuice Oil Tank** device.
...
2. Set your tank capacity in Settings
3. Completed the login flow (including solving any CAPTCHAs)
4. Clicked "Fetch Tank Data" or waited for auto-refresh

### Oil Remaining Shows 0
Set your **Tank Capacity** in Settings. If the scraper only finds the percentage level, it calculates litres from your capacity setting.
...
For issues or feature requests, please visit the [GitHub repository](https://github.com/finalbillybong/boilerjuice-homeassistant-addon).
